package dev.project.scholar_ai.repository.core.websearch;
...
public interface WebSearchOperationRepository extends JpaRepository<WebSearchOperation, String> {
     ...
     */
    @Query(
            "SELECT w FROM WebSearchOperation w WHERE w.projectId = :projectId AND w.status = 'COMPLETED' ORDER BY w.completedAt DESC")
     ...
     */
    @Query(
            "SELECT w FROM WebSearchOperation w WHERE w.status IN ('SUBMITTED', 'IN_PROGRESS') ORDER BY w.submittedAt ASC")
     ...
     */
    @Query(
            "SELECT w FROM WebSearchOperation w WHERE w.status IN ('SUBMITTED', 'IN_PROGRESS') AND w.submittedAt < :beforeTime")
     ...
     */
    @Query(
            "SELECT COALESCE(SUM(w.totalPapersFound), 0) FROM WebSearchOperation w WHERE w.projectId = :projectId AND w.status = 'COMPLETED'")
