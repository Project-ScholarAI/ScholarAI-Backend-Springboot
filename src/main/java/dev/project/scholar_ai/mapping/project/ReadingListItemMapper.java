...
import dev.project.scholar_ai.model.core.project.ReadingListItem;
...
public interface ReadingListItemMapper {
    ...
    @Mapping(target = "paperId", ignore = true) // Don't update paper ID
    @Mapping(
            target = "status",
            expression =
    ...
    @Mapping(target = "readingProgress", source = "dto.readingProgress")
    @Mapping(target = "readCount", ignore = true) // Don't update read count
    @Mapping(target = "isBookmarked", source = "dto.isBookmarked")
